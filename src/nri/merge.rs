...
fn ensure_linux(
...
) -> &mut nri_api::LinuxContainerAdjustment {
    ...
    adjustment
        .linux
        .as_mut()
        .expect("linux adjustment initialized")
}
...
fn ensure_linux_resources(
    ...
    linux
        .resources
        .as_mut()
        .expect("linux resources initialized")
}
...
fn upsert_key_value(values: &mut Vec<nri_api::KeyValue>, key: &str, value: nri_api::KeyValue) {
...
fn upsert_cdi_device(values: &mut Vec<nri_api::CDIDevice>, key: &str, value: nri_api::CDIDevice) {
...
fn upsert_rlimit(values: &mut Vec<nri_api::POSIXRlimit>, key: &str, value: nri_api::POSIXRlimit) {
...
fn set_optional_int(
    target: &mut MessageField<nri_api::OptionalInt>,
    value: &MessageField<nri_api::OptionalInt>,
) {
    *target = value.clone();
}
...
fn merge_resources(
    ...
    if let Some(memory) = resources.memory.as_ref() {
        ...
        if memory.disable_oom_killer.is_some() {
            claim_simple(
                owners,
                container_id,
                nri_api::Field::MemDisableOomKiller,
                plugin,
            )?;
    ...
    if let Some(cpu) = resources.cpu.as_ref() {
        ...
        if cpu.realtime_runtime.is_some() {
            claim_simple(
                owners,
                container_id,
                nri_api::Field::CPURealtimeRuntime,
                plugin,
            )?;
        ...
        if cpu.realtime_period.is_some() {
            claim_simple(
                owners,
                container_id,
                nri_api::Field::CPURealtimePeriod,
                plugin,
            )?;
    ...
    if let Some(blockio_class) = resources.blockio_class.as_ref() {
        ...
        set_optional_string(
            &mut ensure_linux_resources(merged).blockio_class,
            &MessageField::some(blockio_class.clone()),
        );
    }
    if let Some(rdt_class) = resources.rdt_class.as_ref() {
        claim_simple(owners, container_id, nri_api::Field::RdtClass, plugin)?;
        set_optional_string(
            &mut ensure_linux_resources(merged).rdt_class,
            &MessageField::some(rdt_class.clone()),
        );
    }
...
fn merge_linux_adjustment(
    ...
    for namespace in &linux.namespaces {
        ...
        upsert_namespace(
            &mut ensure_linux(merged).namespaces,
            &ns_type,
            namespace.clone(),
        );
    }
    for (key, value) in &linux.sysctl {
        ...
        if removed {
            ...
            ensure_linux(merged)
                .sysctl
                .insert(mark_for_removal(&name), String::new());
    ...
    for (key, value) in &linux.net_devices {
        ...
        ensure_linux(merged)
            .net_devices
            .remove(&mark_for_removal(&name));
        ensure_linux(merged).net_devices.insert(name, value.clone());
    }
    ...
    if let Some(rdt) = linux.rdt.as_ref() {
        if rdt.remove {
            ...
            if rdt.enable_monitoring.is_some() {
                ...
                set_optional_bool(&mut merged_rdt.enable_monitoring, &rdt.enable_monitoring);
            }
    ...
    for (plugin, adjustment) in plugins {
        ...
        if let Some(hooks) = adjustment.hooks.as_ref() {
            ...
            if !hooks.create_runtime.is_empty() {
                merged_hooks
                    .create_runtime
                    .extend(hooks.create_runtime.clone());
                changed = true;
            }
            if !hooks.create_container.is_empty() {
                merged_hooks
                    .create_container
                    .extend(hooks.create_container.clone());
                changed = true;
            }
            if !hooks.start_container.is_empty() {
                merged_hooks
                    .start_container
                    .extend(hooks.start_container.clone());
                changed = true;
            }
...
mod tests {
    ...
    fn merges_annotation_removal_then_replacement_for_same_plugin() {
        ...
        first
            .annotations
            .insert(mark_for_removal("a"), String::new());
    ...
    fn merges_update_resources_for_target_and_side_effects() {
        ...
        let target_cpu = result
            ...
            .unwrap();
        assert_eq!(
            target_cpu.shares.as_ref().map(|value| value.value),
            Some(512)
        );
        let target_memory = result
            ...
            .unwrap();
        assert_eq!(
            target_memory.limit.as_ref().map(|value| value.value),
            Some(4096)
        );
